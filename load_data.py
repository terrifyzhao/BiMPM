...
def load_data(file):
    ...
    # label = [l.astype(np.float32) for l in label]
    ...
    p_vec = list(map(lambda x:w2v_process(x), p_vec))
    h_vec = list(map(lambda x:w2v_process(x), h_vec))
